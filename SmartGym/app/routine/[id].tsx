        ...
        <Card style={styles.section}>
          ...
          <View style={styles.colorRow}>
            {ROUTINE_COLORS.map((c) => (
              ...
              >
                {color === c && <Icon name="checkmark" size={14} color={Colors.textOnAccent} />}
              </TouchableOpacity>
            ))}
          </View>
        </Card>
                ...
                <View style={styles.exInputRow}>
                  {(
                    [
                      ...
                      { label: `Weight (${weightUnit})`, field: 'weight' },
                    ...
                    <View key={field} style={styles.exInputGroup}>
                      ...
                      <TextInput
                        ...
                        value={
                          field === 'weight'
                            ? (ex.weight ? getDisplayWeight(ex.weight, weightUnit).toString() : '0')
                            : String((ex as unknown as Record<string, unknown>)[field] ?? 0)
                        }
                        onChangeText={(v) => {
                          if (field === 'weight') {
                            const parsed = parseFloat(v) || 0;
                            const weightInKg = getInputWeightInKg(parsed, weightUnit);
                            updateExField(idx, 'weight', weightInKg);
                          } else {
                            updateExField(idx, field, v);
                          }
                        }}
