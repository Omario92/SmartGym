...
import { getDisplayWeight, formatWeight } from '@/lib/unit';
...
const SwipeableSessionCard: React.FC<{
  ...
  const displayVolume = weightUnit === 'lbs' ? Math.round((session.totalVolume ?? 0) * 2.20462) : (session.totalVolume ?? 0);
  ...
  return (
    <View style={styles.swipeContainer}>
      ...
      <View style={styles.deleteReveal}>
        <Icon name="trash" size={22} color={Colors.textOnDark} />
      </View>
            ...
            <View style={styles.sessionStats}>
              {session.duration != null && (
                <View style={styles.sessionStat}>
                  <Icon name="time-outline" size={12} color={Colors.iconInactive} />
              ...
              {session.totalSets != null && (
                <View style={styles.sessionStat}>
                  <Icon name="layers-outline" size={12} color={Colors.iconInactive} />
                  <Text style={{ fontSize: FontSize.xs, marginLeft: 3, color: Colors.iconInactive }}>
              ...
              {session.totalVolume != null && session.totalVolume > 0 && (
                <View style={styles.sessionStat}>
                  <Icon name="barbell-outline" size={12} color={Colors.iconInactive} />
                  <Text style={{ fontSize: FontSize.xs, marginLeft: 3, color: Colors.iconInactive }}>
...
export default function HistoryScreen() {
  ...
  const strengthHistory = useMemo(() => {
    if (!selectedStrengthEx) return [];
    const raw = getExercise1RMHistory(useStore.getState(), selectedStrengthEx);
    return raw.map(item => ({
      ...item,
        ...
        <View style={styles.headerActions}>
          {sessions.length > 0 && (
            <TouchableOpacity style={styles.headerBtn} onPress={() => setShowClearModal(true)}>
              <Icon name="trash-outline" size={18} color={Colors.iconInactive} />
            </TouchableOpacity>
          )}
          <TouchableOpacity
          ...
          >
            <Icon name="calendar-outline" size={18} color={Colors.iconInactive} />
          </TouchableOpacity>
        </View>
      </View>
          ...
          <Card style={{ marginHorizontal: 0 }}>
            ...
            {prExerciseIds.length === 0 ? (
              ...
              <>
                ...
                {selectedStrengthEx && (() => {
                  const pr = exercisePRs[selectedStrengthEx];
                  const displayOneRM = weightUnit === 'lbs' ? Math.round(pr.oneRM * 2.20462) : pr.oneRM;
                  const displayWeight = weightUnit === 'lbs' ? Math.round(pr.weight * 2.20462) : pr.weight;
                  return (
                    <View>
                      <View style={styles.prHeader}>
                        <Text color="muted">Current 1RM PR</Text>
                        <Text variant="h3" color="accent">{displayOneRM} {weightUnit}</Text>
                        <Text color="muted" style={{ fontSize: FontSize.xs }}>
                          Best Set: {displayWeight}{weightUnit} × {pr.reps}
                        </Text>
                      </View>
                      <LineChart 
            ...
            {unlockedAwards.map((award) => {
              ...
              return (
                ...
                >
                  ...
                  {!award.unlocked && (
                    <Icon name="lock-closed" size={14} color={Colors.iconInactive} style={{ marginTop: Spacing.xs }} />
                  )}
                </View>
              );
            })}
