...
export async function syncAllUserData(userId: string) {
  ...
  try {
    ...
    // 2. Push Dirty / Deleted Routines FIRST — so a local delete or edit reaches
    //    the cloud before we pull (otherwise the pull would resurrect it).
    ...
    for (const r of dirtyRoutines) {
      if (r.syncStatus === 'deleted') {
        // Hard delete: routine_exercises are removed via ON DELETE CASCADE.
        if (r.cloudId) {
          await supabase.from('routines').delete().eq('id', r.cloudId);
        } else {
          // No cloud id recorded — remove any cloud rows matching this local id.
          await supabase.from('routines').delete().eq('user_id', userId).eq('local_id', r.id);
        }
        // Drop the local tombstone so it can never come back.
        store.removeRoutineLocal(r.id);
      } else {
    ...
    // 3. Pull Routines (exclude soft-deleted rows) and merge.
    const { data: cloudRoutines, error: rErr } = await supabase
      .from('routines')
