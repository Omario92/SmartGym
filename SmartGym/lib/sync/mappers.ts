...
export function routineToRow(routine: Routine, userId: string, cloudId?: string): Partial<RoutineRow> {
  return {
    ...
    // Persist the archived flag in metadata (no dedicated column) so it survives
    // across reloads/devices.
    metadata: { archived: !!routine.archived },
...
export function rowToRoutine(row: RoutineRow, exercises: RoutineExercise[]): Routine {
  return {
    ...
    exercises,
    // Carry sync metadata so pulls don't create duplicates or lose archived/deleted state.
    cloudId: row.id,
    updatedAt: row.updated_at,
    archived: !!(row.metadata && row.metadata.archived),
